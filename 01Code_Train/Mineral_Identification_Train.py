...
def main(_):
    ...
    bottleneck_tensor, jpeg_data_tensor = tf.import_graph_def(
        graph_def1, return_elements=[BOTTLENECK_TENSOR_NAME, JPEG_DATA_TENSOR_NAME]
    )
    # 定义输入
    bottleneck_input = tf.placeholder(
        tf.float32, [None, BOTTLENECK_TENSOR_SIZE], name="BottleneckPlaceholder"
    )
    # 定义输出
    ground_truth_input = tf.placeholder(
        tf.float32, [None, n_classes], name="GroundTruthnput"
    )

    # 定义第一层全连接
    weights_1 = tf.Variable(
        tf.truncated_normal([BOTTLENECK_TENSOR_SIZE, 2048], stddev=0.01),
        name="Weights_1",
    )
    ...
    weights_2 = tf.Variable(
        tf.truncated_normal([2048, 2048], stddev=0.01), name="Weights_2"
    )
    ...
    weights_3 = tf.Variable(
        tf.truncated_normal([2048, n_classes], stddev=0.01), name="Weights_3"
    )
    ...
    cross_entropy = tf.nn.softmax_cross_entropy_with_logits_v2(
        logits=logits_3, labels=ground_truth_input
    )
    cross_entropy_mean = tf.reduce_mean(cross_entropy)
    global_step = tf.Variable(0)
    learning_rate = tf.train.exponential_decay(
        LEARNING_RATE, global_step, 100, 0.9995, staircase=True
    )
    ...
    with tf.Session() as sess:
        ...
        for i in range(STEPS):
            ...
            _, cross, training_accuracy = sess.run(
                [train_step, cross_entropy_mean, evaluation_step],
                feed_dict={
            ...
            if i % 50 == 0:
                ...
                print(
                    "Step %d:Validation accuracy on random sampled"
        ...
        test_accuracy, final_tensor = sess.run(
            [evaluation_step, final_tensor],
            feed_dict={
        ...
        print(
            "Step %d:Test accuracy on random sampled"
            "%d examples=%.1f%%" % (i, BATCH, test_accuracy * 100)
        )
